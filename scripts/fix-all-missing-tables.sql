...
-- Run this on the server: PGPASSWORD=tableshare_secure_pass_2026 psql -h localhost -U tableshare_user -d tableshare_prod -f scripts/fix-all-missing-tables.sql
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '✅ message_text column already exists in messages table';
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS conversation_prompts (
  ...
  context VARCHAR(50) DEFAULT 'any', -- 'first_message' or 'any'
  cuisine_type VARCHAR(100), -- Optional: filter by cuisine
  time_of_day VARCHAR(20), -- Optional: 'morning', 'afternoon', 'evening', 'night'
  usage_count INTEGER DEFAULT 0,
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Create indexes for conversation_prompts
...
-- 4. Insert seed data for conversation_prompts if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM conversation_prompts LIMIT 1) THEN
    ...
    ('How is everything going?', 'general', 'any');
    ...
    ('What''s the best meal you''ve had this week?', 'food', 'any'),
    ('Are you a foodie? What''s your favorite restaurant?', 'food', 'any');
    ...
    ('What''s the vibe like here?', 'restaurant', 'any'),
    ('Have you tried their signature dish?', 'restaurant', 'any');
    ...
    ('What''s your favorite way to spend a weekend?', 'fun', 'any'),
    ('Any fun plans coming up?', 'fun', 'any');

    -- Deep prompts
    INSERT INTO conversation_prompts (prompt_text, category, context) VALUES
    ('What''s something you''re passionate about?', 'deep', 'any'),
    ('What motivates you in life?', 'deep', 'any'),
    ('What''s a goal you''re working towards?', 'deep', 'any'),
    ('What makes you happy?', 'deep', 'any');

    RAISE NOTICE '✅ Inserted seed data for conversation_prompts';
  ELSE
    RAISE NOTICE '✅ conversation_prompts table already has data';
  END IF;
END $$;
...
SELECT 'conversation_prompts count:' as info;
SELECT COUNT(*) as prompt_count FROM conversation_prompts;

SELECT 'prompt_usage count:' as info;
SELECT COUNT(*) as usage_count FROM prompt_usage;

RAISE NOTICE '✅ All tables and columns fixed!';
